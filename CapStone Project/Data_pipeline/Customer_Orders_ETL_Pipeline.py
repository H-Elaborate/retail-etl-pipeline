...
DATA_DIR = BASE_DIR.parent / "Data" #No matter where I run this from, always use the script’s folder.
...
customers = pd.read_csv(DATA_DIR / "customers.csv") #DataFrame\
products  = pd.read_csv(DATA_DIR / "products.csv") #DataFrame\
orders    = pd.read_csv(DATA_DIR / "orders.csv") #DataFrame\
...
# Create the database directory and connect
(BASE_DIR.parent / "Database").mkdir(parents=True, exist_ok=True)
conn = sqlite3.connect(BASE_DIR.parent / "Database" / "retails_sales.db")
